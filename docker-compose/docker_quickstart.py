...
BLOCK_PRINTING_TAGS = ["blockprinting", "blockprintedfabric", "blockprintedsaree"]
...
with smart_run(session):
    ...
    session.set_action_delays(
        enabled=True, like=10, comment=5, follow=4.17, unfollow=28
    )
    ...
    session.set_comments(COMMENT_LIST, media="Photo")
    ...
    session.follow_user_followers(
        BIG_ACCOUNTS, amount=20, randomize=True, interact=True
    )
    ...
    session.follow_commenters(
        BIG_ACCOUNTS, amount=5, daysold=365, max_pic=3, sleep_delay=600, interact=True
    )
